  ...
  >
    ...
    <div class="absolute inset-x-0 bottom-24 h-24 z-30 flex items-center overflow-x-auto scrollbar-hide px-8 pb-4">
      <TransitionGroup name="fleet">
        ...
        >
          <div class="text-4xl filter">{{ getVehicleIcon((history.question as any).colorName || '') }}</div>
...
const getVehicleName = (colorName: string) => (VEHICLE_MAP as any)[colorName]?.name || colorName
const getVehicleIcon = (colorName: string) => (VEHICLE_MAP as any)[colorName]?.icon || '🚗'
const getHiraganaColor = (colorName: string) => (COLOR_HIRA_MAP as any)[colorName] || colorName

const getVehicleOnomatopoeia = (colorName: string) => {
    const sounds = (VEHICLE_MAP as any)[colorName]?.sound || ['ぶーん！']
...
const getVehicleAnimation = (colorName: string) => {
  const map = VEHICLE_MAP as Record<string, { move: string }>
  const baseMove = map[colorName]?.move || 'animate-drive'
  if (colorName === '赤') {
