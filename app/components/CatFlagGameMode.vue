...
const hasMultipleVariants = (colorName: string) => {
  return colorName === '赤'
}

const randomizeVariant = (colorName: string) => {
...
const currentColorEn = computed(() => {
  const target = props.userAnswer || props.currentQuestion
  return (COLOR_TO_EN as Record<string, string>)[target?.colorName || ''] || 'red'
})
...
const handleVideoError = (e: Event) => {
  ...
  console.warn('Flag video load failed:', target.src)
