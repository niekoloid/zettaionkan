...
const playbackPart = ref<Tone.Part | null>(null)
const playbackMode = ref<'chord' | 'single'>('chord')
const singleNotePosition = ref<'low' | 'mid' | 'high'>('high')
...
const getTransformedChord = (baseChord: Chord | null | undefined, position: string): Chord | null => {
...
const currentChord = computed<Chord | null>(() => {
...
const activeNotes = computed<Set<string>>(() => {
  if (currentChordIndex.value === -1) return new Set<string>()
  ...
  if (step.chord) {
    ...
    if (transformed) {
      ...
      const notesToPlay: string[] = playbackMode.value === 'single' 
...
const startPlayback = async () => {
  ...
  try {
    ...
    if (!samplers[selectedInstrument.value]) {
      await loadSampler(selectedInstrument.value as 'yamaha' | 'steinway')
    }
    ...
    playbackPart.value = new Tone.Part((time: number, event: PlaybackEvent) => {
      ...
      if (event.notes) {
        ...
        if (transformed) {
          const noteIndex = singleNotePosition.value === 'high' ? 2 : (singleNotePosition.value === 'mid' ? 1 : 0)
          const notes: string[] = playbackMode.value === 'single' 
            ? [transformed.notes[noteIndex]!]
            : transformed.notes
...
const isLightColor = (hex: string | undefined) => {
  if (!hex) return false
    ...
    <div class="min-h-screen flex flex-col max-w-3xl mx-auto relative overflow-hidden">
      ...
      <main class="flex-grow px-6 py-8 flex flex-col items-center">
        ...
        <div class="flex-grow w-full flex flex-col items-center justify-center relative">
          ...
          <div 
            v-if="selectedSong.sequence[0] && !selectedSong.sequence[0].notes"
          ...
          <div v-if="playbackMode === 'single'" class="flex flex-col items-center space-y-3">
            <div class="flex bg-gray-50 p-1 rounded-xl border border-gray-100 scale-90">
              <button 
                ...
                :key="pos.k"
                @click="singleNotePosition = pos.k as 'low' | 'mid' | 'high'"
