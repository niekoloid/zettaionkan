...
const userAnswer = ref<Chord | null>(null)
...
const whiteKeyChords = computed(() => QUIZZ_CHORDS.value.filter(c => (c.sortOrder || 0) <= 9))
const blackKeyChords = computed(() => QUIZZ_CHORDS.value.filter(c => (c.sortOrder || 0) > 9))
const currentLayoutChords = computed(() => {
  if (view.value === 'quiz') {
    return shuffledIds.value.map(id => QUIZZ_CHORDS.value.find(c => c.id === id)).filter((c): c is Chord => !!c) as Chord[]
  }
  return QUIZZ_CHORDS.value.filter(c => selectedChordIds.value.has(c.id))
})
...
const moveNext = () => {
  ...
  if (nextChord) questions.value.push(nextChord)
