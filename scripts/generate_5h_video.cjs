...
const CONFIG = {
  ...
  outputDir: path.join(__dirname, '../videos/outputs_5h'),
...
async function generate() {
  ...
  for (let L = 1; L <= 14; L++) {
    ...
    // 最終結合
    const finalOutput = path.join(CONFIG.outputDir, `Lv${L}_5h_continuous.mp4`);
    console.log(`🎬 Level ${L} 結合中... (5時間版)`);
    const concatCmd = `ffmpeg -y -f concat -safe 0 -i "${listPath}" -c copy "${finalOutput}"`;
    execSync(concatCmd, { stdio: 'ignore' });

    console.log(`✨ Level ${L} (5時間版) 完成！: ${finalOutput}`);
  }
}
